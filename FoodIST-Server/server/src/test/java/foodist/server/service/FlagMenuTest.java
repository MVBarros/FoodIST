...
public class FlagMenuTest {
    ...
    public void validTest() {
        stub.flagMenu(Contract.FlagMenuRequest.newBuilder().setCookie(cookie).setMenuId(menuId).build());
        assertEquals(impl.getMenus().get(menuId).getFlagCount(), 1);
    }
    ...
    public void invalidCookie() {
        ...
        try {
            stub.flagMenu(Contract.FlagMenuRequest.newBuilder().setMenuId(menuId).build());
        } catch (StatusRuntimeException e) {
            assertEquals(e.getStatus(), Status.UNAUTHENTICATED);
            assertEquals(impl.getMenus().get(menuId).getFlagCount(), 0);
            throw e;
        }
    }
    ...
    public void invalidPhotoId() {
        exceptionRule.expect(StatusRuntimeException.class);
        try {
            stub.flagMenu(Contract.FlagMenuRequest.newBuilder().setCookie(cookie).setMenuId(10).build());
        } catch (StatusRuntimeException e) {
