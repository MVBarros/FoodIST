...
public class CancelQueueJoinTest {
	...
	public void validTest() {
		stub.cancelQueueJoin(Contract.QueueRequest.newBuilder().setFoodService(SERVICE).setUuid(UUID).build());
	...
	public void noSuchUUIDTest() {
		stub.cancelQueueJoin(Contract.QueueRequest.newBuilder().setFoodService(SERVICE).setUuid(UUID2).build());
		Map<String, QueuePosition> queue = impl.getService(SERVICE).getQueue();
	...
	public void noSuchServiceTest() {
		stub.cancelQueueJoin(Contract.QueueRequest.newBuilder().setFoodService(SERVICE2).setUuid(UUID).build());
